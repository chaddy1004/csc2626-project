...
class SAC:
    def __init__(self, n_states, n_actions, ratio, offline=True, cql=False, iS=False, bc=False):
        # Test
        self.ratio = ratio
        self.offline = offline
        self.cql = cql # SAC + CQL (offline)
        self.iS = iS # SAC + CQL with importance sampling (iS)
        self.bc = bc # behaviour cloning (baseline)
        ...
        # TODO: merge experience replays into one using SumTree memory
        self.experience_replay_on = deque(maxlen=self.replay_size)
    ...
    def train_actor(self, ep, s_currs, a_currs, sample_action, log_action_probs):
        if self.bc or ep < self.policy_eval_start:
    ...
    def train_critic(self, value, s_currs, a_currs, r, s_nexts, dones):
        ...
        if self.cql:
            ...
            if self.iS: # importance sampling between random density and expected actor policy
                ...
                q1_term_pre_exp = torch.cat([rand_value_1, pred_value_1_next, pred_value_1], 1)
                q2_term_pre_exp = torch.cat([rand_value_2, pred_value_2_next, pred_value_2], 1)
            else:
                q1_term_pre_exp = torch.cat([rand, predicts.unsqueeze(1), next_actions, curr_actions], 1)
                q2_term_pre_exp = torch.cat([rand2, predicts2.unsqueeze(1), next_actions2, curr_actions2], 1)
            ...
            # Bellman + CQL
            loss = loss + loss_cql_1
            loss2 = loss2 + loss_cql_2
    ...
    def process_batch_on(self, x_batch):
    ...
    def train(self, x_batch, ep):
        if self.offline:
            ...
            s_currs, a_currs, r, s_nexts, dones = self.process_batch_on(x_batch=x_batch)
        ...
        if not self.bc:
            ...
            loss, loss2 = self.train_critic(value=value, s_currs=s_currs, a_currs=a_currs, r=r, s_nexts=s_nexts, dones=dones)
            self.update_weights()
        else:
            loss = 0.0
            loss2 = 0.0
...
def main(episodes_off, episodes_on, exp_name, overfit, cql, iS, bc):
    ...
    agent = SAC(n_states=n_states, n_actions=n_actions, ratio=(0.0, 1.0),
                offline=True, cql=cql, iS=iS, bc=bc)
    log_freq = 10
    exploration_eps = -1
    for ep in range(episodes_off): # offline
    ...
    for ep in range(episodes_on): # online
        ...
        while not done:
            ...
            if done:
                ...
                if ep > 0 and ep % log_freq == 0:
                    writer.add_scalar("score", score, ep + episodes_off)
                    writer.add_scalars("training loss", {"loss": losses[0].item(),
                                                        "loss2": losses[1].item(),
                                                        "loss_actor": losses[2].item(),
                                                        "alpha_loss": losses[3].item()}, ep + episodes_off)
    writer.close()
    return agent
...
if __name__ == "__main__":
    ...
    ap.add_argument("--exp_name", type=str, default="sac_on_off", help="exp_name")
    ap.add_argument("--episodes_off", type=int, default=5000, help="number of episodes to run offline")
    ap.add_argument("--episodes_on", type=int, default=5000, help="number of episodes to run online")
    ap.add_argument("--overfit", action="store_true", help="number of episodes to run")
    ap.add_argument("--cql", action="store_true", help="run cql (offline setting)")
    ap.add_argument("--IS", action="store_true", help="run cql with IS (offline setting)")
    ap.add_argument("--bc", action="store_true", help="run bc baseline (offline setting)")
    args = vars(ap.parse_args())
    trained_agent = main(episodes_off=args["episodes_off"],
