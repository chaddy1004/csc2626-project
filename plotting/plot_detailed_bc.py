...
df_sac_bl.insert(0, "Ratio", ["1.0" for _ in range(len(df_bc_1.index))], allow_duplicates=False)

df_sac_bl.plot.line(x='Step', y='Value', ax=ax, color="dimgray", alpha=1)
df_bc_0.plot.line(x='Step', y='Value', ax=ax, color="dodgerblue", alpha=1)
df_bc_1.plot.line(x='Step', y='Value', ax=ax, color="dodgerblue", alpha=0.5)

labels = ["SAC (online)", "BC 0.0", "BC 1.0"]
...
YMIN = -500
YMAX = 400
plt.title('Scores of BC during training, detailed', fontsize=16)
plt.ylim(YMIN, YMAX)
plt.ylabel("Episodic Score")
plt.xlabel("Episode")
# plt.show()
plt.savefig("bc_detailed.pdf")
